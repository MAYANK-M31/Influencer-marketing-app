...
const ImageReviewScroll = ({ route, navigation }) => {
            ...
            <SafeAreaView style={{ height: HEIGHT, backgroundColor: "white" }} >
                ...
                <View style={style.header} >
                    ...
                    <View style={style.heading} >
                        <Text style={{ fontSize: 18, fontWeight: "bold", color: "white" }} >Photo</Text>
                    </View>
                ...
                <ScrollView style={{ backgroundColor: "black" }} horizontal={true} pagingEnabled={true} decelerationRate={'fast'} scrollEventThrottle={16} snapToInterval={WiDTH}  >
                    < View style={{ width: WiDTH, height: "100%" }} >
                        <Image resizeMode={"contain"} style={{ width: "100%", height: "100%" }} source={{ uri: route.params.item.uri }} />
                    </View>
                    {route.params.items.map((items) =>
                        <>
                            {route.params.item.uri !== items.uri ?
                                <View style={{ width: WiDTH, height: "100%" }} >
                                    <Image resizeMode={"contain"} style={{ width: "100%", height: "100%" }} source={{ uri: items.uri }} />
                                </View>
                                :
                                null
                            }
...
export default ImageReviewScroll;
